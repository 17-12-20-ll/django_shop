...
class UserAction(models.Model):
    ...
    用户浏览行为
    """
    user = models.ForeignKey(User, verbose_name='所属用户行为', related_name='user', on_delete=models.CASCADE)
    goods = models.ForeignKey(Goods, verbose_name='所看商品', related_name='goods', on_delete=models.CASCADE)
    click = models.IntegerField(null=False)
    add_time = models.DateTimeField(auto_now_add=True, verbose_name='添加时间')
    last_time = models.DateTimeField(auto_now=True, verbose_name='最后浏览时间')

    class Meta:
        db_table = 'f_user_active'
